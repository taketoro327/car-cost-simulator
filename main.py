...
st.markdown("""
    <style>
    .block-container { max-width: 800px; padding_top: 2rem; }
    .stMetric { background_color: rgba(128, 128, 128, 0.1); padding: 15px; border_radius: 10px; }
    [data_testid="stMetricValue"] { font_size: 2rem !important; }
    .streamlit_expanderContent { font_size: 0.85rem; line_height: 1.6; }
    div[role="radiogroup"] label p { font_size: 0.85rem !important; }
    </style>
...
# アクセスカウンター
st.markdown(
    "![Visitors](https://komarev.com/ghpvc/?username=kenja-car-v7&label=Visitors&color=red&style=flat-square)"
)

# --- 1. 基本条件入力 ---
with st.container(border=True):
    st.markdown("<h3 style='font_size: 1.2rem; margin_bottom: 0.5rem;'>🗓️ シミュレーションの基本条件</h3>", unsafe_allow_html=True)
...
# --- 3. 車両比較入力 ---
st.markdown("<h3 style='font_size: 1.2rem; margin_bottom: 0.5rem;'>🚘 比較する車両の入力</h3>", unsafe_allow_html=True)
...
with col_v1:
    with st.container(border=True):
        st.markdown("<h4 style='font_size: 1.1rem;'>【A】軽自動車</h4>", unsafe_allow_html=True)
...
with col_v2:
    with st.container(border=True):
        st.markdown("<h4 style='font_size: 1.1rem;'>【B】普通車</h4>", unsafe_allow_html=True)
...
# --- 4. 結果表示 ---
st.divider()
st.markdown("<h3 style='font_size: 1.2rem; margin_bottom: 0.5rem;'>📊 算出結果（トータルコスト）</h3>", unsafe_allow_html=True)
...
with st.expander("🧮 賢者の計算根拠・物理法則と市場ファクト"):
    # --- 【修正】タイトルの文字サイズを 1.2rem -> 1.0rem に縮小 ---

    st.markdown("<h3 style='font_size: 1.0rem; margin_top: 0px;'>1. 将来の予想売却価格（残価率）の算定基準</h3>", unsafe_allow_html=True)
    ...
    st.markdown("<h3 style='font_size: 1.0rem;'>2. 基本維持費の計算式と単価設定</h3>", unsafe_allow_html=True)
    ...
    * **タイヤ関連費（消耗＋スタッドレス導入費＋毎年の履き替え工賃）**:
    ...
    st.markdown("<h3 style='font_size: 1.0rem;'>3. 機械的負荷・物理法則による補正</h3>", unsafe_allow_html=True)
    st.markdown("""
    ...
    * **高速道路利用の影響**: 軽自動車は高速走行時の負荷が顕著なため、利用頻度に応じてメンテナンス係数を動的に加算しています。
    ...
    st.markdown("<h3 style='font_size: 1.0rem;'>4. 13年超え重課税と10万kmメンテナンスの壁</h3>", unsafe_allow_html=True)
    st.markdown("""
