...
with st.container(border=True):
    # 【修正1】st.subheaderに変更（スマホでの文字改行防止）
    ...
    with col_base2:
        ...
        # 【修正3】保険プランの文字切れ解消（選択肢を短くし、helpで詳細説明）
        insurance_options = [
            "基本プラン（車両保険なし）", 
            "安心プラン（車両保険エコノミー）", 
            "万全プラン（車両保険一般）"
        ...
        ins_type = st.selectbox(
            ...
            help="【プラン詳細】\n"
                 "- **基本プラン**: 対人対物無制限\n"
                 "- **安心プラン**: 対人対物無制限 ＋ 車両保険(エコノミー)\n"
                 "- **万全プラン**: 対人対物無制限 ＋ 車両保険(一般)"
        )
...
# スマホでは1カラムに自動的に折り返されるように、あえてst.columnsを使わず
# 単純にコンテナを縦に並べる方がスマホUIとしては綺麗になります。
# 今回は元コードの意図（PCでの2カラム表示）を尊重し、そのままにしています。
