...
st.markdown(f"""
    ...
    /* 見出しサイズ統一(1.1rem) */
    .custom-h3 {{ font-size: 1.1rem !important; font-weight: bold; margin-bottom: 0.5rem; }}
...
if icon_base64:
    ...
    header_html = '<div class="custom-h3">🚗 賢者の車選びシミュレーター</div>'

st.markdown(header_html, unsafe_allow_html=True)
st.write("「軽自動車」と「普通車」の購入費・維持費・リセールを、物理法則と市場データに基づきリアルに比較。")

# アクセスカウンター
st.markdown("![Visitors](https://komarev.com/ghpvc/?username=kenja-car-final-v3&label=Visitors&color=red&style=flat-square)")
...
with st.container(border=True):
    ...
    st.divider()
    ins_type = st.radio("保険プラン", options=["基本（車両なし）", "安心（＋車両エコノミー）", "万全（＋車両一般）"], index=1, horizontal=True)

    st.divider()
    col_tire1, col_tire2 = st.columns(2)
    with col_tire1: is_winter = st.toggle("スタッドレスタイヤを使用", value=True)
    with col_tire2: w_months = st.selectbox("冬タイヤ装着期間 (ヶ月)", options=[1, 2, 3, 4, 5, 6], index=2) if is_winter else 0
...
def calc_all(price, mpg, is_kei, age_label, is_resale_included, t_unit, w_price, change_fee):
    ...
    return int(actual_dep + fuel + total_tax + shaken_total + ins_total + maint_total), resale_val, int(actual_dep), int(fuel), int(total_tax), int(shaken_total), int(ins_total), int(maint_total)
...
with col_v2:
    with st.container(border=True):
        ...
        if "15" in s_tire_size: s_t, s_w, s_c = 40000, 40000, 8000
        elif "16" in s_tire_size: s_t, s_w, s_c = 80000, 60000, 10000
        else: s_t, s_w, s_c = 120000, 80000, 12000
        s_total, s_resale, s_dep, s_fuel, s_tax, s_shaken, s_ins, s_tire = calc_all(s_p, s_m, False, s_age, is_resale_included, s_t, s_w, s_c)
...
with res_c1:
    ...
    with st.expander("🔍 内訳"): st.write(f"- 車両実質負担: {k_dep:,}円\n- ガソリン代: {k_fuel:,}円\n- 自動車税等: {k_tax:,}円\n- 車検代: {k_shaken:,}円\n- 任意保険: {k_ins:,}円\n- メンテ・タイヤ: {k_tire:,}円")
with res_c2:
    st.metric("普通車 合計", f"{s_total:,}円")
    with st.expander("🔍 内訳"): st.write(f"- 車両実質負担: {s_dep:,}円\n- ガソリン代: {s_fuel:,}円\n- 自動車税等: {s_tax:,}円\n- 車検代: {s_shaken:,}円\n- 任意保険: {s_ins:,}円\n- メンテ・タイヤ: {s_tire:,}円")
...
with st.expander("🧮 賢者の計算根拠・物理法則と市場ファクト"):
    ...
    res_text = f"（購入価格 － {years}年後の予想売却価格）" if is_resale_included else "（購入価格のみ）"
    st.markdown(f"""
    * **車両実質負担額**: {res_text} をベースに計算。
    * **燃料代**: 走行距離 ÷ 実用燃費 × ガソリン単価（{gas}円）で算出。
    * **自動車税**: 軽自動車 10,800円/年、普通車 30,500円/年（1.5L以下）。
    ...
    * **タイヤ関連費（消耗＋スタッドレス導入費＋毎年の履き替え工賃）**:
    ...
    st.markdown("""
    * **高回転による金属疲労**: 660ccの軽自動車は巡航時の回転数が普通車の1.5〜2倍になります。摩擦回数が多いことによるオイル劣化や駆動系の摩耗を考慮し整備費を補正。
    ...
    * **10万km目前の大物パーツ整備**: 累計10万km到達時に発生する「タイミングベルト、ウォーターポンプ、足回り」等の交換費用（軽:10万、普通:12万）を特別整備費として算入。
    """)
